#!/usr/bin/env python3
"""Update logbook by adding detailed July 2026 section before Upcoming section."""

# Read the file
with open('LOGBOOK-LatestUpdateOn-May15th.md', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# Find the "Upcoming" section header
upcoming_start_idx = None
for i, line in enumerate(lines):
    if '## Upcoming' in line:
        upcoming_start_idx = i
        break

if upcoming_start_idx is None:
    print("ERROR: Could not find '## Upcoming' section in logbook")
    exit(1)

# Define the new July section with detailed entries
july_section = """## July 2026
...
| 08 Jul 2026 | Wednesday | Network Infrastructure         | Enhanced CORS configuration (6 commits, 20+ files) to dynamically detect/support captive portal scenarios. Implemented network utility function `getPreferredLocalIP()` intelligently selecting best local IP address. Refactored authentication logic and implemented redirect URL builder for captive portal scenarios. Added QR code type toggle to PaymentCounterView. Enhanced Vite configuration and created improved QR code download functionality. Ensured system works on various local network configs. |
...
# Insert July section before Upcoming
new_lines = lines[:upcoming_start_idx] + [july_section + "\n"] + lines[upcoming_start_idx:]

# Write back
with open('LOGBOOK-LatestUpdateOn-May15th.md', 'w', encoding='utf-8') as f:
    f.writelines(new_lines)

print("Successfully added detailed July 2026 section to logbook!")
