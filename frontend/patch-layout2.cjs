...
// 2. Remove bottom nav completely
let bottomNavStart = cv.indexOf('{/* ============ BOTTOM NAV (mobile-app feel) ============ */}');
if (bottomNavStart !== -1) {
  let bottomNavEnd = cv.indexOf('</nav>', bottomNavStart) + 6;
  cv = cv.substring(0, bottomNavStart) + cv.substring(bottomNavEnd);
}

// 3. Remove Sticky category menu
let catStart = cv.indexOf('{/* Sticky category icons (Grab-style, moved from top of page) */}');
if (catStart !== -1) {
  let catEnd = cv.indexOf('{loading ? (', catStart);
  cv = cv.substring(0, catStart) + cv.substring(catEnd);
}
...
// 5. Close the main and wrapper div at the end of the file.
// We must find the VERY last </div>. We can do this using regex to replace the final `</div>\n  );\n};`
cv = cv.replace(/<\/div>\s*\);\s*};\s*$/, '        </main>\n      </div>\n    </div>\n  );\n};\n');

fs.writeFileSync('src/components/garden/CustomerView.tsx', cv);
console.log('CustomerView layout successfully patched to Sidebar!');
